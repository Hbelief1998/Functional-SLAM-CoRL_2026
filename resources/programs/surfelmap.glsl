...
void main(void) {
  ...
  int stride = max(render_stride, 1);
  int render_width = (int(width) + stride - 1) / stride;
  int y = (gl_PrimitiveIDIn / render_width) * stride;
  int x = (gl_PrimitiveIDIn - (gl_PrimitiveIDIn / render_width) * render_width) * stride;
