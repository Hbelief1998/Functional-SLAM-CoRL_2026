...
class MonocularDataset(torch.utils.data.Dataset):
    ...
    def read_img(self, idx):
        ...
        if isinstance(img_path, pathlib.Path):
    ...
    def truncate(self, max_frames):
        if max_frames is None or max_frames < 0:
            return
        if hasattr(self, "rgb_files") and len(self.rgb_files) > max_frames:
            self.rgb_files = self.rgb_files[:max_frames]
        if hasattr(self, "timestamps") and len(self.timestamps) > max_frames:
            self.timestamps = self.timestamps[:max_frames]
        if hasattr(self, "total_frames"):
            self.total_frames = min(self.total_frames, max_frames)
...
class FunGraphDataset(MonocularDataset):
    def __init__(self, dataset_path):
        ...
        self.timestamps = []
        # Check if we should use consecutive timestamps (for rgb_SLAM subset)
        is_rgb_slam = "rgb_SLAM" in str(self.dataset_path)

        if False: # is_rgb_slam:
             self.timestamps = np.arange(0, len(self.rgb_files)).astype(self.dtype) / 30.0
        else:
            fallback_fps = 30.0
            fps_match = re.search(r"(\d+(?:\.\d+)?)fps", str(self.dataset_path), re.IGNORECASE)
            if fps_match:
                try:
                    fallback_fps = max(float(fps_match.group(1)), 1e-6)
                except ValueError:
                    fallback_fps = 30.0
            for f in self.rgb_files:
                # Assumes frame_XXXXX.jpg
                match = re.search(r"frame_(\d+)", f.name)
                if match:
                    idx = int(match.group(1))
                    self.timestamps.append(idx / 30.0)
                else:
                    self.timestamps.append(len(self.timestamps) / fallback_fps)
...
def load_dataset(dataset_path):
    ...
    split_lower = [part.lower() for part in split_dataset_type]
    path = pathlib.Path(dataset_path)
    ext = path.suffix.lower().lstrip(".")
    if path.is_file() and ext in ["mp4", "avi", "mov"]:
        return MP4Dataset(dataset_path)
    if "tum" in split_lower:
        return TUMDataset(dataset_path)
    if "euroc" in split_lower:
        return EurocDataset(dataset_path)
    if "eth3d" in split_lower:
        return ETH3DDataset(dataset_path)
    if "7-scenes" in split_lower:
        return SevenScenesDataset(dataset_path)
    if any("fungraph" in part for part in split_lower):
        return FunGraphDataset(dataset_path)
    if "realsense" in split_lower:
        return RealsenseDataset()
    if "webcam" in split_lower:
        return Webcam()

    ext = split_dataset_type[-1].split(".")[-1].lower()
