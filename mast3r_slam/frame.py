...
class SharedStates:
    def __init__(self, manager, h, w, dtype=torch.float32, device="cuda"):
        ...
        # Node-aware place recognition: shared signatures & debug log.
        # All entries must be JSON-serializable plain Python objects
        # (dict / list / str / int / float / bool / None).
        self.node_kf_signatures = manager.dict()
        self.node_current_query_signature = manager.dict()
        self.node_place_debug = manager.list()
        self.runtime_profile_events = manager.list()
    ...
    # --- Node-aware place recognition: shared signatures ----------------
    def set_kf_node_signature(self, kf_idx, signature):
        """Store a JSON-serializable signature for a keyframe slot."""
        if signature is None:
            return
        with self.lock:
            self.node_kf_signatures[int(kf_idx)] = dict(signature)

    def get_kf_node_signature(self, kf_idx):
        with self.lock:
            sig = self.node_kf_signatures.get(int(kf_idx))
            return dict(sig) if sig else None

    def get_kf_node_signatures(self, kf_indices):
        out = {}
        with self.lock:
            for k in (kf_indices or []):
                key = int(k)
                if key in self.node_kf_signatures:
                    out[key] = dict(self.node_kf_signatures[key])
        return out

    def get_all_kf_node_signatures(self):
        """Return a plain-dict snapshot of all keyframe node signatures."""
        out = {}
        with self.lock:
            for k in list(self.node_kf_signatures.keys()):
                try:
                    out[int(k)] = dict(self.node_kf_signatures[k])
                except Exception:
                    continue
        return out

    # Optional aliases used by some callers/tests.
    def set_kf_place_signature(self, kf_idx, signature):
        return self.set_kf_node_signature(kf_idx, signature)

    def get_kf_place_signature(self, kf_idx):
        return self.get_kf_node_signature(kf_idx)

    def set_current_node_query_signature(self, signature):
        with self.lock:
            self.node_current_query_signature.clear()
            if signature:
                for k, v in dict(signature).items():
                    self.node_current_query_signature[k] = v

    def get_current_node_query_signature(self):
        with self.lock:
            if not self.node_current_query_signature:
                return {}
            return dict(self.node_current_query_signature)

    def clear_current_node_query_signature(self):
        with self.lock:
            try:
                self.node_current_query_signature.clear()
            except Exception:
    ...
    def append_node_place_debug(self, payload, max_items: int = 200):
        if not payload:
            return
        with self.lock:
            self.node_place_debug.append(dict(payload))
            # Trim oldest entries to bound memory.
            try:
                while len(self.node_place_debug) > int(max_items):
                    self.node_place_debug.pop(0)
            except Exception:
                # Manager proxies may behave oddly under heavy contention;
                # we silently bail rather than crash the SLAM loop.
                pass

    def append_runtime_profile_event(self, payload, max_items: int = 20000):
        if not payload:
            return
        with self.lock:
            self.runtime_profile_events.append(dict(payload))
            try:
                while len(self.runtime_profile_events) > int(max_items):
                    self.runtime_profile_events.pop(0)
            except Exception:
                pass

    def get_runtime_profile_events(self):
        with self.lock:
            return [dict(e) for e in list(self.runtime_profile_events)]
