...
try:
    from transformers.modeling_utils import find_pruneable_heads_and_indices, prune_linear_layer  # old API
except ImportError:
    try:
...
try:
    ...
    # Newer Transformers versions dropped this helper, so we keep a local copy
    def apply_chunking_to_forward(forward_fn, chunk_size, chunk_dim, *input_tensors):
        if chunk_size <= 0:
            return forward_fn(*input_tensors)
        if len(input_tensors) == 0:
            raise ValueError("apply_chunking_to_forward expects at least one tensor input")

        tensor_shape = input_tensors[0].shape[chunk_dim]
        if tensor_shape % chunk_size != 0:
            raise ValueError(
                f"The input dimension {tensor_shape} is not a multiple of chunk_size {chunk_size}."
            )

        num_chunks = tensor_shape // chunk_size
        input_tensors_chunked = tuple(tensor.chunk(num_chunks, dim=chunk_dim) for tensor in input_tensors)

        output_chunks = []
        for i in range(num_chunks):
            chunk_inputs = tuple(tensor_chunk[i] for tensor_chunk in input_tensors_chunked)
