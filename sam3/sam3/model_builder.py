...
def _create_vit_backbone(compile_mode=None, img_size: int = 1008):
...
def _create_transformer_decoder(image_size: int = 1008) -> TransformerDecoder:
...
def _create_vision_backbone(
    ...
    # ViT backbone
    vit_backbone: ViT = _create_vit_backbone(compile_mode=compile_mode, img_size=image_size)
...
def _load_checkpoint(model, checkpoint_path):
    ...
    # Drop buffers whose shape depends on input image_size (freqs_cis, relative_coords).
    # They are precomputed at module construction time from the runtime input_size, so
    # any saved values for a different image_size would cause size mismatches at load.
    model_sd = model.state_dict()
    for _k in list(sam3_image_ckpt.keys()):
        if _k.endswith(".freqs_cis") or _k.endswith(".relative_coords"):
            cur = model_sd.get(_k)
            if cur is not None and tuple(cur.shape) != tuple(sam3_image_ckpt[_k].shape):
                del sam3_image_ckpt[_k]
